...
void tree_barrier(treenode_t *treenode) {
    ...
    while(!children_not_ready(treenode)) {
        ...
        for(i=0; i<4; i++) {
            if(treenode->childid_arrive[i] != -1)
                MPI_Irecv(&buf, 1, MPI_INT, treenode->childid_arrive[i], 1, MPI_COMM_WORLD, &mpi_request);
            if(buf == I_M_READY)
    ...
    if(treenode->processid != 0) {
        ...
        MPI_Send(&buf, 1, MPI_INT, parentid_a, 1, MPI_COMM_WORLD);
        printf("Processor %d: Signaled I_M_READY to my parent.\n", treenode->processid);
    }

    // Reset childnotready array in preparation for the next barrier.
    for(i=0; i<4; i++)
        treenode->childnotready[i] = 4*treenode->processid+i < num_procs ? 1 : -1;
    ...
    if(treenode->processid != 0) {
        while(1) {
            ...
            MPI_Irecv(&buf, 1, MPI_INT, parentid_w, 1, MPI_COMM_WORLD, &mpi_request);
    ...
    printf("Processor %d: Sent wakeup signal to children.\n", treenode->processid);
...
int main(int argc, char *argv[]) {
    ...
    int i;
    for(i=0; i<4; i++) {
        treenode.childid_arrive[i] = 4*id+i < num_procs;
        treenode.childnotready[i] = 4*id+i < num_procs ? 1 : -1;
    }
